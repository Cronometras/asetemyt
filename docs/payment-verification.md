...
Las cuatro variables están configuradas como secretos en producción en Pages / asetemyt / Settings / Variables and Secrets. Las claves privadas no se guardan en Git ni en el entorno local:

- `STRIPE_SECRET_KEY`: clave restringida «Asetemyt · Cloudflare producción», con escritura de Checkout y cupones, y lectura de clientes, suscripciones, facturas, productos, precios y saldo.
...
- `STRIPE_WEBHOOK_SECRET`: configurado con el secreto del endpoint `https://asetemyt.com/api/stripe/webhook`.
- `RESEND_API_KEY`: clave «Asetemyt Cloudflare producción», limitada a envío desde asetemyt.com. El dominio figura verificado en Resend.
